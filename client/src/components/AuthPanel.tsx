...
// Helper: Get backend API URL from env (Vite provides import.meta.env)
...
export const AuthPanel: React.FC<AuthPanelProps> = ({ onAuthSuccess }) => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data && event.data.auth_code) {
        ...
        setTimeout(async () => {
          setIsLoading(true);
          try {
            const result = await validateAuthCode(event.data.auth_code, config);
            if (result.success) {
  ...
  // Generate Fyers OAuth URL using backend (POST with credentials)
  const handleGenerateAuthUrl = async () => {
    ...
    setIsLoading(true);
    try {
      // Use the authService to generate the URL
          ...
          if (popupUrl && popupUrl.includes('trade.fyers.in/api-login/redirect-uri')) {
            ...
            if (authCodeFromUrl && authCodeFromUrl !== authCode) {
              ...
              setTimeout(async () => {
                setIsLoading(true);
                try {
                  const result = await validateAuthCode(authCodeFromUrl, config);
                  if (result.success) {
                    setMessage({ type: 'success', text: 'Authentication successful!' });
                    onAuthSuccess();
                  } else {
            ...
            if (bodyText && bodyText.includes('authorization code')) {
              ...
              if (authCodeMatch && authCodeMatch[0] !== authCode) {
                ...
                setTimeout(async () => {
                  setIsLoading(true);
                  try {
                    const result = await validateAuthCode(extractedCode, config);
                    if (result.success) {
                      setMessage({ type: 'success', text: 'Authentication successful!' });
                      onAuthSuccess();
                    } else {
  ...
  // Validate auth code using backend (POST with credentials)
  const handleValidateAuthCode = async () => {
    ...
    setMessage(null);
    try {
      // Use the authService to validate the code
      const result = await validateAuthCode(authCode, config);
      if (result.success) {
        setMessage({ type: 'success', text: 'Authentication successful!' });
        onAuthSuccess();
      } else {
