...
export const StrategyMonitoringCard: React.FC<StrategyMonitoringCardProps> = ({
    ...
    if (liveData) {
      ...
      if (hmaConfig && lastCandleClose !== null) {
        ...
        if (crossover === 'ABOVE' && tradeStatus.status === 'WAITING') {
          ...
          // Calculate target and stop loss based on type
          let targetPrice: number;
          let stopLossPrice: number;
          
          if (targetType === 'PERCENTAGE') {
            targetPrice = entryPrice * (1 + targetPoints / 100);
          } else {
            targetPrice = entryPrice + targetPoints;
          }
          
          if (stopLossType === 'PERCENTAGE') {
            stopLossPrice = entryPrice * (1 - stopLossPoints / 100);
          } else {
  ...
  }, [liveData, hmaConfig, lastCandleClose, tradeStatus.status, quantity, targetPoints, stopLossPoints, targetType, stopLossType, onTradeLog, symbol]);
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center">
                  ...
                  Target {targetType === 'PERCENTAGE' ? `(${targetPoints}%)` : `(${targetPoints} pts)`}
                </label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1 flex items-center">
                  ...
                  Stop Loss {stopLossType === 'PERCENTAGE' ? `(${stopLossPoints}%)` : `(${stopLossPoints} pts)`}
                </label>
