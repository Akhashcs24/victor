...
export class OrderService {
   ...
   */
  static async placeOrder(orderData: FyersOrderData): Promise<FyersOrderResponse> {
    try {
      ...
      // Log order data in development mode
      this.logOrderData(orderData, 'Placing Order');
  ...
  /**
   * Get order history from backend API
   */
  static async getOrderHistory(): Promise<any[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to get order history');
      }
  ...
  static async getPositions(): Promise<any[]> {
    try {
      ...
      const response = await fetch(`${API_URL}/api/orders/positions`, {
        headers: {
          'Authorization': accessToken
        }
      });
