...
function App() {
  ...
  const handleContractInputChange = (field: keyof ContractInputs, value: any) => {
    ...
    if (field === 'ceSymbol' || field === 'peSymbol') {
      ...
      if (oldSymbol && oldSymbol !== value) {
        ...
        import('./services/hmaService').then(({ HMAService }) => {
          HMAService.clearCache(oldSymbol);
        });
      }
    }
    
    TradingService.updateContractInputs({ [field]: value });
    setTradingState(TradingService.getTradingState());
  ...
  const handleAddToMonitoring = async () => {
    const { ceSymbol, peSymbol, ceLots, peLots, targetPoints, stopLossPoints, entryMethod } = tradingState.contractInputs;
    ...
    try {
      ...
      if (ceSymbol) {
        promises.push(
          MultiSymbolMonitoringService.addSymbolToMonitoring(
            ceSymbol, 'CE', ceLots, targetPoints, stopLossPoints, entryMethod
          )
        );
      }
      ...
      if (peSymbol) {
        promises.push(
          MultiSymbolMonitoringService.addSymbolToMonitoring(
            peSymbol, 'PE', peLots, targetPoints, stopLossPoints, entryMethod
          )
        );
      }
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                  ...
                  <div className="mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-3">Trading Mode</label>
                        <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border border-slate-200">
                          <div className="flex items-center gap-3">
                            <div className={`px-3 py-1 rounded-md text-sm font-medium transition-colors ${
                          ...
                          <label className="relative inline-flex items-center cursor-pointer">
                            ...
                            />
                            <div className="relative w-14 h-7 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-green-500"></div>
                            <span className="ml-3 text-sm font-medium text-slate-700">
                              {tradingState.tradingMode === 'LIVE' ? 'Live' : 'Paper'}
                            </span>
                          </label>
                        </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">Target (Points)</label>
                        <input 
                          type="number" 
                          placeholder="50"
                          ...
                          className="w-full px-4 py-3 bg-white border border-slate-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover:border-slate-400 transition-colors text-slate-900 placeholder-slate-500"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-slate-700 mb-2">Stop Loss (Points)</label>
                        <input 
                          type="number" 
                          placeholder="30"
