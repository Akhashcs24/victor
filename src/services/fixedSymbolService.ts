...
export class FixedSymbolService {
  ...
  /**
   * Format expiry date for symbol based on Fyers API requirements.
   * Uses YYMMM format (e.g., 25JUN) for all index options.
   */
  private static formatExpiryForSymbol(expiryDate: Date): string {
  ...
  /**
   * Generate strike symbols for CE and PE options - Updated for new format
   */
  static fixedSymbolService (
    ...
    // Always use current expiry (no more expiry dropdown)
    const currentDate = new Date();
    const expiryDateObj = this.getNextExpiryDate(mappedIndex, currentDate);
    const expiryType = this.EXPIRY_TYPES[mappedIndex as keyof typeof this.EXPIRY_TYPES] || 'weekly';
    const expiryString = this.formatExpiryForSymbol(expiryDateObj);
