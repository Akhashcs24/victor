...
export class HMAService {
  ...
  private static startLiveMonitoring(symbol: string): void {
    ...
    if (!cache || cache.isLiveMonitoring) return;

    cache.isLiveMonitoring = true;
    cache.monitoringInterval = setInterval(() => {
      this.updateHMAWithNewCandle(symbol);
    }, 60 * 1000); // Update every minute

    console.log(`🟢 Live HMA monitoring started for ${symbol}`);
  }
  ...
  private static stopLiveMonitoring(symbol: string): void {
    const cache = this.candleCache.get(symbol);
    if (cache && cache.monitoringInterval) {
      clearInterval(cache.monitoringInterval);
      cache.isLiveMonitoring = false;
      console.log(`🔴 Live HMA monitoring stopped for ${symbol}`);
    }
  }

  /**
   * Update HMA with new candle data during live monitoring
   */
  private static async updateHMAWithNewCandle(symbol: string): Promise<void> {
    ...
    try {
      ...
      const now = new Date();
      const fiveMinutesAgo = new Date(now.getTime() - 5 * 60 * 1000);
      const fromDate = this.formatDateForAPI(fiveMinutesAgo);
      const toDate = this.formatDateForAPI(now);
      ...
      if (historicalData && historicalData.length > 0) {
        const newCandles = this.convertAndFilterTradingHoursCandles(historicalData);
        
        if (newCandles.length > 0) {
          const latestNewCandle = newCandles[newCandles.length - 1];
          
          // Check if this is a new candle (not already in cache)
